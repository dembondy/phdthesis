...
The definition of $\eta$ takes the time horizon of service provision into account. This is done, so that the performance assessment gives a result that is scaled to the time scale of service delivery. For example, if two aggregators perform with an error of equal magnitude, but one aggregator is contracted to deliver the service on a shorter time horizon, the assessment of this aggregator should be worse than the one which was contracted for a longer period. Conversely, if two aggregators deliver a service on different time horizons, and both have error in service delivery which is \emph{proportionately} the same, the performance assessment will evaluate them to have equal performance. This is illustrated in Figure~\ref{fig:indextest1}, where 5 aggregators deliver a reference tracking service over different time horizons. All aggregators have the same proportionate error\footnote[][-2\baselineskip]{This is ensured by using a sine as the ``actual'' performance of the aggregators, and each case is an extra period of the sine wave.} which leads to the same $\eta$, as can be seen in Table~\ref{tab:qostest1}. There is a slight increase in the values of $\eta$ due to numerical round off.\bondy{should I also show an example of an evaluation where the QoS is different?}
\begin{figure}
\centering
\subfloat[Simulated service delivery]{\includegraphics[width=1\columnwidth]{indextest.eps}%
\label{subfig:indextest1}} \\
\subfloat[QoS for the services]{\includegraphics[width=1\columnwidth]{qostest.eps}%
\label{subfig:qostest1}}
\caption{Test of the QoS definition, where 5 aggregators deliver the same service with same performance.}
...
\begin{table}[h!]
	...
	\begin{tabular}{cc}
		...
		\bottomrule
	\end{tabular}
	\caption{The values of $\eta$ over different service delivery horizons.}
	\label{tab:qostest1}
\end{table}
...
Following the same example of $\eta$, the verification index scales with time, and the verification index values for the examples shown in previous section can be seen in Table~\ref{tab:epstest1}. Contrary to the service performance assessment index, the verification index does not have a clear limit for what is considered a verified serviced. For some services, e.g. ancillary services, it is critically important that $QoS(t) \lll 1$, which would mean a requirement of $\epsilon \approx 0}$. In other cases, $\epsilon > 0}$ is tolerable to certain extent. The tolerance limit for $\epsilon$ should be defined in the contract agreements between aggregator and the entity acquiring the services.
\begin{table}[h!]
	\centering
	\begin{tabular}{cc}
		\toprule
		...
		\bottomrule
	\end{tabular}
	\caption{The values of $\epsilon$ over different service delivery horizons.}
	\label{tab:epstest1}
\end{table}
